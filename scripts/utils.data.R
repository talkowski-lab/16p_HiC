...
is_h_ideal <- function(Resolution, h){
    if (is.numeric(Resolution)) {
        Resolution <- format(Resolution, scientific=FALSE, trim=FALSE)
    } 
...
load_hicrep_results <- function(resolutions){
    ...
    tibble(fileinfo=.) %>%
    mutate(filepath=glue('{HICREP_RESULTS_DIR}/{fileinfo}')) %>%
    mutate(fileinfo=str_remove(fileinfo, '-hicrep.txt')) %>% 
    # Get hicrep params
    separate_wider_delim(
        fileinfo,
        delim='/',
    ...
    separate_wider_delim(
        ...
        names=
            c(
              'A.SampleID', NA, 'A.ReadFilter', NA, NA,
              'B.SampleID', NA, 'B.ReadFilter', NA
            ),
    ) %>%
    mutate(
        ReadFilter=
            case_when(
                A.ReadFilter == B.ReadFilter ~ A.ReadFilter,
                A.ReadFilter != B.ReadFilter ~ 'mixed'
            )
    ) %>% 
    # Remove redundant pairs 
    filter(A.SampleID != B.SampleID) %>% 
    rowwise() %>% 
    mutate(pair=list(paste0(sort(c(A.SampleID, B.SampleID))))) %>% 
    mutate(
        across(
            ends_with('.SampleID'), 
            .fns=list("Genotype"=get_genotype),
            .names="{.fn}.{.col}"
        )
    ) %>% 
    mutate(
        Genotype.Pair=
            c(Genotype.A.SampleID, Genotype.B.SampleID) %>%
            sort(decreasing=TRUE) %>%
            paste0(collapse='.vs.') #%>% list()
    ) %>%
    ungroup() %>% 
    distinct(is.Downsampled, h, Resolution, A.ReadFilter, B.ReadFilter, pair, .keep_all=TRUE) %>%
    { 
...
run_multiHiCCompare <- function(
    ...
    rowwise() %>% 
    mutate(filepath=glue('{MATRIX_DIR}/{sample_ID}.{format(resolution, scientific=FALSE)}.{region}.txt')) %>%
    mutate(
        sparse.matrix=
            filepath %>% 
...
load_all_multiHiCCompare_results <- function(
    ...
    tibble(filename=.) %>%
    mutate(filepath=glue('{multiHiCCompare_results_dir}/{filename}')) %>%
