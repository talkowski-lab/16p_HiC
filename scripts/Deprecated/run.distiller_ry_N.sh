...
mkdir -p "${LOG_DIR}"
CONDA_ENV="nextflow"
export PATH="/PHShome/ry077/miniforge3/:$PATH"
...
main() {
    for yml_file in ${@}; do
        ...
        [[ -f "${log_file}.out" ]] && rm ${log_file}.{out,err}
        cmd="bash -l -c module load wget
module unload java perl
...
${CONDA_DIR}/envs/${CONDA_ENV}/bin/nextflow run ${DISTILLER_FILE} -params-file ${yml_file} -w /scratch/r/ry077"
        sbatch \
            --partition ${PARTITION} \
            --time ${TIME} \
            --mem ${MEM} \
