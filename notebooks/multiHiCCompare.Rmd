...
```{r plot_fc_dist_gw, results='asis', fig.dim=c(9, 6), eval=TRUE}
# dist.plot.df
dist.plot.df %>% 
    mutate(
        statistic=
...
```{r plot_volcanos_free, results='asis', fig.dim=c(16, 8), eval=TRUE}
differential.contacts.df %>% 
    volcano_plot_fnc(
        group.cols=c('resolution', 'Celltype'),
        color.var='Sample.Group', 
...
```{r plot_distances_gw, results='asis', fig.dim=c(9, 6), eval=TRUE}
distance.plot.df %>%
    distance_plot_fnc(
        group.cols=c('resolution', 'Celltype'),
...
```{r plot_distances_chr, results='asis', fig.dim=c(14, 8), eval=TRUE}
distance.plot.df %>%
    distance_plot_fnc(
        group.cols=c('resolution', 'Celltype'),
        legend.position='top',
        ...
        plot.fnc=
            function(freq.df, text.size=7){
                ...
                theme(
                    ...
                    strip.text=element_text(size=text.size+3)
                ) + 
                make_ggtheme()
            }
    )
