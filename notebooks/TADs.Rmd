...
Load all TAD Annotations
```{r load_TADs}
# library(furrr); plan(multisession, workers=length(availableWorkers()) / 2)
# library(furrr); plan(multisession, workers=6)
...
tad.annotations <- 
    ...
    mutate(
        ...
        isMerged=factor(ifelse(grepl('Merged', SampleNumber), 'Merged', 'Individual'))
    )
tad.annotations %>% head(1) %>%  t()
```
Columns for TAD annotations:
- Sample Attributes
  - Sample.ID
  - Edit
  - Genotype
...
```{r PLOT_cooltools_tad_number, results='asis', fig.width=8, fig.height=6}
cooltools.annotations %>% 
    mutate(resolution=scale_numbers(resolution)) %>% 
...
tad.annotations %>% 
    rowwise() %>% 
    mutate(nTADs=TADs %>% nrow()) %>% 
    ungroup() %>% 
    summarize(nTADs=sum(nTADs))
tad.annotations %>% 
    rowwise() %>% 
    mutate(nTADs=TADs %>% filter(is_boundary) %>% nrow()) %>% 
    ungroup() %>% 
    summarize(nTADs=sum(nTADs))
tad.annotations %>% 
    rowwise() %>% 
    mutate(nTADs=TADs %>% filter(is_boundary) %>% nrow()) %>% 
    group_by(method, resolution, weight) %>% 
    summarize(as_tibble_row(summary(nTADs)))
```
