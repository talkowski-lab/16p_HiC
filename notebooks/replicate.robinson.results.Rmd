...
```
```{r load_metadata, message=FALSE, rows.print=5, eval=FALSE}
...
regions.of.interest %>%
    mutate(region.dist=sprintf('%0.1f Kb', region.dist/1000)) %>% 
...
print(c(telomere.start, telomere.end, deletion.start, deletion.end))
...
```{r due_diligence, eval=FALSE}
annotated.contacts.df %>% count(ReadFilter, resolution, chr, weight)
# annotated.contacts.df %>% filter(is.na(ReadFilter)) %>% t()
annotated.contacts.df %>%
    filter(grepl('WT.Merged', Sample.ID)) %>%
    # count(region)
    filter(region == 'chr16p vs chr16p') %>% 
    # filter(region == 'chr16p.deletion vs chr16p.telomere') %>% 
    count(range2) %>% print(n=Inf)
...
plot.df <- 
    ...
    # filter(between(contact.dist, dist.min, dist.max)) %>% 
    filter(
    ...
    bind_rows()
plot.df %>% dplyr::count(Sample.ID, IF.Threshold, region.title)
```
...
```{r basic_individual, results='asis', message=FALSE, fig.height=7, fig.width=7} 
plot.df %>% 
...
```{r basic_merged, results='asis', message=FALSE, fig.height=4, fig.width=6} 
plot.df %>% 
    filter(ReplicateNum == 'Merged') %>% 
    filter(
        region.title %in% c(
...
```{r extra_regions_merged, results='asis', message=FALSE, fig.width=12, fig.height=8, eval=FALSE} 
plot.df %>% 
    filter(ReplicateNum == 'Merged') %>% 
    filter(
        region.title %in% c(
