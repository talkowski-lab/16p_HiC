...
```{r plot_pct_pairs, fig.dim=c(6,4)}
pairs.plot.df %>% 
...
```{r plot_pair_orientation, results='asis', fig.dim=c(12,6)}
pair.orientation.df %>% 
    ...
    make_nested_plot_tabs(
        ...
        legend.position.inside=c(0.15, 0.70), # c(0,0) bottom left, c(1,1) top-right.
        scales='fixed',
        x.scale.mode='log10',
        y.scale.mode='log10',
        facet.ncol=3
...
```{r plot_cis_trans, results='asis', fig.dim=c(8,6)}
plot.df %>% 
    make_nested_plot_tabs(
        plot.fnc=plot_heatmap,
