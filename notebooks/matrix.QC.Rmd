...
pairs.df <- make_pair_qc_df(pair.stats.list)
pairs.plot.df <- 
    pairs.df %>% 
    ...
    mutate(isMerged=ifelse(grepl('Merged', SampleNumber), 'Merged', 'Individual'))
```

## Number of pairs 

```{r plot_n_pairs, echo=FALSE, fig.align='center', fig.height=5}
pairs.plot.df %>% 
    ...
    plot_qc_barplot(
        ...
        facet_row='isMerged',
        scales='free_y',
        scale_y_method='log10'
    )
```

## % of pairs 

```{r plot_pct_pairs, echo=FALSE, fig.align='center', fig.height=5}
pairs.plot.df %>% 
    rename('Unique Pairs'=frac) %>% 
    add_lines_qc_barplot(
        y_val='Unique Pairs',
        fill_col='Sample.ID',
        facet_col='Celltype',
        facet_row='isMerged',
